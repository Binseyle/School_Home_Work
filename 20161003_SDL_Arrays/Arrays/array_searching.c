...
size_t array_bin_search2(int array[],size_t left, size_t right, int x )
{
    if (left == right)
    {
        return right;
    ...
    if (x<array[mid])
    {
        return array_bin_search2(array,left,mid,x);
    }
    else
    {
        return array_bin_search2(array,mid+1,right,x);
    }
}
...
size_t array_bin_search(int array[], size_t len, int x)
{
    size_t left = 0 ;
    size_t right = len-1;
    size_t j  =  (array_bin_search2(array,left,right,x));
    return j;
}
